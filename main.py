        ...
        while True:
            ...
            for i in range(device_count):
                ...
                try:
                    ...
                    total_fb_memory.labels(device=i, hostname=hostname).set(mem_info.total)
                    free_fb_memory.labels(device=i, hostname=hostname).set(mem_info.free)
                    used_fb_memory.labels(device=i, hostname=hostname).set(mem_info.used)

                    # log.info('Obtaining utilization statistics...')
                    utilization = nvmlDeviceGetUtilizationRates(handle)
                    # log.info('Utilization statistics = %s', str(utilization))

                    gpu_utilization.labels(device=i, hostname=hostname).set(utilization.gpu / 100.0)
                    memory_utilization.labels(device=i, hostname=hostname).set(utilization.memory / 100.0)
                except Exception as e:
                    log.info(e, exc_info=True)

                log.info('Pushing metrics to gateway at %s...', args.gateway)
                push_to_gateway('my-prom-prometheus-pushgateway:9091', job=platform.node(), registry=core.REGISTRY)
